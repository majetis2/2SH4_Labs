...
void TestQ1_addZero(CuTest *tc) {

	int n = 5;
	double input1[5] = {3.60, -2.00, 3.30, -10.00, 1.00};
	double input2[5] = {-3.60, 2.00, -3.30, 10.00, -1.00};
	double actual[5];
...
void TestQ2_false_zero(CuTest *tc) {
	...
	int input[3][3] = {{0, 1, -1}, {6, 8, 0}, {3, 5, 9}};
	int actual = is_diag_dom(input);
	int expected = 0;
	CuAssertIntEquals(tc, expected, actual);
}
...
void TestQ3_zero_neg(CuTest *tc) {
	int n = 3;
	int input[3][3] = {{0, -1, 0},{ -1, 0, -1},{ -1, -1, -1}};
	int expected[9]= {0, -1, -1, -1, 0, 0, -1, -1, -1};
	int actual[9];
	diag_scan(input,actual);
...
void TestQ4_allSame_capital(CuTest *tc) {

	const char input[] = "BbBBb";
	int actual[26]={0};
...
void TestQ7_NoEqual(CuTest *tc) {

	// HARDCODED testcases
   	int input_val1[] = {11,44,22};
	int input_pos1[] = {1,2,6};
	int input_val2[] = {66,55,33};
	int input_pos2[] = {3,4,5};
	...
	int expected_output_val[6]={11, 44, 66, 55, 33, 22};
	int expected_output_pos[6]={1, 2, 3, 4, 5, 6};
...
void TestQ7_ZeroSum(CuTest *tc) {
	...
	int expected_output_val[6]={11, 55,0,0,0};
	int expected_output_pos[6]={1, 4,0,0,0};
...
CuSuite* Lab2GetSuite() {
	...
	SUITE_ADD_TEST(suite,TestQ1_addZero);
